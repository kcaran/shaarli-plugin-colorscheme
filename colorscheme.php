...
{
    $brightness = sqrt(.299 * pow(hexdec($match[1]), 2)
        + .587 * pow(hexdec($match[2]), 2)
        + .114 * pow(hexdec($match[3]), 2));
...
function display_color($match)
{
    ...
    return '<a class="colorscheme' . $dark
...
function hook_colorscheme_render_linklist($data)
{
    ...
    foreach ($data['links'] as &$value) {
        $value['description'] = preg_replace_callback(
            $pattern,
            "display_color",
            $value['description'],
            -1,
